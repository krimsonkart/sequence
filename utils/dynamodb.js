...
function hasUnprocessedItems(batchWriteResponse) {
    return (
        ...
        Object.keys(batchWriteResponse.UnprocessedItems).length
    );
}
...
async function persistDbEntries(puts, tableName, { awsCredentials } = {}) {
    ...
    for (const batch of batches) {
        ...
        do {
            ...
            batchWriteResponse = await getDocClient(awsCredentials).batchWrite(itemsToProcess).promise();
...
function deleteDbEntry(deleteRequest, tableName) {
    logger.log({ params: deleteRequest }, `Deleted entry`);
...
async function batchDeleteDbEntries(deleteRequests, tableName) {
    ...
    // Shuffle the deletion in hope of not having a heated partition.
    const batches = _.chunk(_.shuffle(deleteRequests), 20);
    await Promise.map(
        batches,
        async batch => {
            ...
            do {
                try {
                    ...
                    err.origCode = err.code; // preserve for logging
                    err.code = 'BatchWriteItemFailed';
                    // response is not present in all error conditions (e.g., validation errors)
                    err.unprocessed = batchDeleteResponse && batchDeleteResponse.UnprocessedItems;
                    throw err;
                }
